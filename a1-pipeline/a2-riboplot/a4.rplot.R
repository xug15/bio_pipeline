...
pdf(paste(filename,'.tran.pdf',sep=''),width=6,height=4,paper='special')
mp <- barplot(data$number+1,col= rainbow(3),border = NA,main=name,log="y")
axis(1,at=mp,labels=data$position)
dev.off()

pdf(paste(filename,'.tran.axis.pdf',sep=''),width=6,height=4,paper='special')
mp <- barplot(data$number+1,col= rainbow(3),border = NA,main=name)
axis(1,at=mp,labels=data$position)
dev.off()
...
pdf(paste(filename,'.axis.orf.pdf',sep=''),width=6,height=4,paper='special')
mp <- barplot(data$number[orfstart:orfend],col= rainbow(3),border = NA,main=name)
axis(1,at=mp,labels=data$position[orfstart:orfend])
dev.off()
